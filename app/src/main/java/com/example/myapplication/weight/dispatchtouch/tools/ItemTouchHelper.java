package com.example.myapplication.weight.dispatchtouch.tools;
...
import com.example.myapplication.weight.dispatchtouch.Adapter;
...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
        ...
        if (recyclerView != null) {
            ...
            mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
                ...
                public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                    ...
                    if (newState == RecyclerView.SCROLL_STATE_DRAGGING && lastSelected != null) {
                        closeSwipe();
                    }
                }
    ...
    private synchronized void closeSwipe() {

        Adapter.ItemHolder itemHolder = (Adapter.ItemHolder) lastSelected;
        ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(itemHolder.tv, "translationX", itemHolder.tv.getTranslationX(), 0f);
        ...
        objectAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationStart(Animator _animation) {
                ...
                endRecoverAnimation(lastSelected, true);
                if (mPendingCleanup.remove(lastSelected.itemView)) {
                    mCallback.clearView(mRecyclerView, lastSelected);
                }
            }
    ...
    void checkSelectForSwipe(int action, MotionEvent motionEvent, int pointerIndex) {
        ...
        if (lastSelected != null && lastSelected != vh)
            closeSwipe();
    ...
    public interface ViewDropHandler {
        ...
        /**
         * Called by the {@link androidx.recyclerview.widget.ItemTouchHelper} after a View is dropped over another View.
         * <p>
    ...
    public abstract static class Callback {
         ...
         *                    it is cleared.
         * @param actionState One of {@link androidx.recyclerview.widget.ItemTouchHelper#ACTION_STATE_IDLE},
         *                    {@link androidx.recyclerview.widget.ItemTouchHelper#ACTION_STATE_SWIPE} or
         *                    {@link androidx.recyclerview.widget.ItemTouchHelper#ACTION_STATE_DRAG}.
            ...
            switch (action) {
                case MotionEvent.ACTION_MOVE: {
                    ...
                    if (activePointerIndex >= 0) {
                        ...
                        distanceX = Math.abs(downX - event.getX());
    ...
    private void clickChild(MotionEvent event) {
        ...
        if (mSelected == null)
            return;
        View child = findClickView(mSelected.itemView, (int) event.getRawX(), (int) event.getRawY());
        if (child == null)
            mSelected.itemView.performClick();
        else {
            child.performClick();
        }
    }

    private View findClickView(View tempView, int rawX, int rawY) {
        View resView = null;
        if (tempView instanceof ViewGroup) {
            int count = ((ViewGroup) tempView).getChildCount();
            for (int i = 0; i < count; i++) {
                View view = ((ViewGroup) tempView).getChildAt(i);
                resView = findClickView(view, rawX, rawY);
                if (resView != null)
                    return resView;
            }
        }
        if (canClick(tempView, rawX, rawY))
            resView = tempView;
        return resView;
    }

    /**
     * 根据手势是否在view内；view是否可见；view是否有点击时间
     * 上诉三个条件去判断是否能点击
     *
     * @param temp
     * @return
     */
    private boolean canClick(View temp, int rawX, int rawY) {
        int[] location = new int[2];
        temp.getLocationOnScreen(location);
        Rect rect = new Rect(location[0], location[1], Math.abs(location[0] + temp.getWidth()), Math.abs(location[1] + temp.getHeight()));
        if (rect.contains(rawX, rawY) && temp.getVisibility() == View.VISIBLE && ViewCompat.hasOnClickListeners(temp)) {
            return true;
        } else
            return false;
    }
